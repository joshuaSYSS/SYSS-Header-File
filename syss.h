...
void V003(){
    string s;
    (cin >> s), (cout << "Hi, " << s << "!\nWelcome to SYSS.\nHave a good day " << s << ". See you " << s << "!\n");
}
void V004(){
    string s, s2;(cin >> s >> s2), (cout << s << '-' << s2 << '\n');
}
void V005(){
    ll n, m, cnt;cin >> n >> m;
    while (m) cnt=n&m, n^=m, m=cnt<<1; cout << n << '\n';
}
void V006(){
    ll n = get(); cout << 1001 - n << '\n';
}
void V325(){
    ll n = get();
    up(2, n + 1, 1)
...
void CF1669D(){
    tcin{
       ll n; str s, temp = ""; cin >> n >> s;
        vector<str> v;
        foreach(c, s) {
            if (c == 'W' && temp != "")v.push_back(temp), temp = "";
            else if(c!='W')temp += c;
    }
    ...
    foreach(x, v) {
        ll r = 0, b = 0;
        foreach(c, x)if (c == 'R')r++; else if (c == 'B')b++;
        if (!r || !b) { cout << "NO\n"; done = 1; break; }
    }
    if (!done)cout << "YES\n";
    }
